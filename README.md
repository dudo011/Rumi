...
- 꽃루미 첫 만남과 별정원 메인 화면
- 첫 방탈출 모험 **사라진 별빛 씨앗**
- 중앙 정원, 반짝이는 연못, 시계꽃 숲, 분수대와 달빛 온실 탐색
- 장면 조사, 인벤토리, 단서 수첩, 장치 퍼즐과 최종 추리
- 이야기 장치에 자연스럽게 적용한 초등학교 5학년 약수·배수 학습
- 포포가 씨앗을 보호했다는 결말과 별빛 꽃 개화
- 경험치 `+110 XP`, 꽃숲 성장 단계 및 완료 횟수 저장
- 닉네임과 진행 상황의 기기 내부 저장

Home 화면의 **사건 조사하기** 버튼은 최종 Episode 1을 바로 실행합니다. 이전의 퀴즈형 Episode, 정원 조사 프로토타입과 기술 검증용 실행 화면은 제거했습니다.

현재 MVP의 구체적인 구현 상태와 다음 작업은 [PROJECT_CONTEXT.md](PROJECT_CONTEXT.md)에 정리되어 있습니다.
